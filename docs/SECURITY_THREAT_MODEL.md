...
| F-2 | **C** | SSM read policy is `/renderpr/secrets/*` (all tenants/repos), not scoped to the current repo prefix → cross-tenant secret read. | `renderpr-stack.ts:158-163` | SaaS (breach), BYOC (cross-repo) |
